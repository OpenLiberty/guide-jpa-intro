...
= Performing CRUD operations using Java Persistence API
...
database. Every entity object is managed by an entity manager that will be injected into the
Enterprise Java Bean (EJB). In this guide, you will learn how to use a container-managed entity manager
which means the EJB container is responsible for opening and closing the entity manager, as well
as managing the transaction boundaries so that you do not need to explicitly begin, commit, or rollback
...
to use Java Transaction API (JTA) transactions. In a Java EE environment, the default is JTA if this 
