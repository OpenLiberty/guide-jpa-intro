...
The application that you will working with is the `Event Manager`, which provides services for creating, retrieving, updating
...
would like to learn more about Embedded Derby, you can do so from the https://db.apache.org/derby/papers/DerbyTut/embedded_intro.html[official website.]

You will use JPA annotations to define a Plain Old Java Object (POJO) class as an entity class whose persistent fields
will be persisted to the datastore. Every entity object has a lifecycle that consists of different states that are managed by
an `EntityManager`. In this application the `EntityManager` is container-managed so the EJB container will be responsible for transaction
