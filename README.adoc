...
Every entity object has a lifecycle that consists of different states that are managed by
...
=======
== Getting started
>>>>>>> 4e5088062890407be53a3f2c41bd8ae625239244
...
stored in the database and is visible in the list of current events.
...
An entity class must be defined before you are able to use JPA to store Java objects in a database. A JPA entity is a Java object whose
non-transient fields will be persisted to the database. Any Plain Old Java Object (POJO) class
...
The `EventDao.java` class has a method for each CRUD operation, lets break down each of these methods.
