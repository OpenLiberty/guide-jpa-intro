...
context that is managed by an EntityManager. In this guide you will learn how to use a container-managed 
EntityManager that will be obtained by the application through injection. This means, that the EntityManagers
lifecycle will be managed transparently by the Java EE container.
...
new entities, fetching entities through the `find()` method and queries, and removing entities.
