...
manager is a container-managed entity manager. In this guide you will use a container-managed
entity manager that is injected into the DAO, placing the burden of opening and closing 
...
The CRUD operations operations are defined in the DAO. To perform these operations by using 
JPA, create the `backendServices/src/main/java/io/openliberty/guides/event/dao/EventDao.java` 
file:
...
class and used to access and persist data. The `@Transactional` annotation is used to declaratively
control the transaction boundaries on the `@RequestScoped` CDI bean. This ensures that the
methods run within the boundaries of an active global transaction, which is why it is not
