...
In the [hotspot file=0]`pom.xml` configuration file, the [hotspot=copyDependencies]`copyDependencies` tag copies the specified third-party [hotspot=dependencies] `dependencies` to a specified location. Multiple dependency parameters can be added to the [hotspot=copyDependenciesConfig]`copyDependencies` configuration. The [hotspot=location]`location` parameter can be added to the copyDependencies to override the default location, which is the lib folder of the target server. 
