...
The persistence unit is defined by the [persistence-unit file=0]`<persistence-unit/>` XML element. The [hotspot=transaction-type file=0]`name` attribute is 
...
[hotspot=EventResource]`EventResource` class to declaratively control the transaction boundaries on the [hotspot=RequestedScoped file=2]`@RequestScoped` CDI bean.
...
The [hotspot=testIndividualRead]`testIndividualRead()` method persists a test event to the database and retrieves the 
