...
In order to use the EntityManager at runtime, it must be injected via the `@PersistenceContext` annotation.

The `createEvent` method stores an instance of the Event entity class in the database. An `IllegalArgumentException` will be thrown if the argument is not an instance of the entity class.
