...
The persistence unit is defined by the [hotspot=7-13]`<persistence-unit />` XML element. The `name` attribute is 
required and is used to identify the persistent unit when using the `@PersistenceContext`
annotation to inject the entity manager later in this guide. The [hotspot=7]`transaction-type="JTA"` 
...
A JTA transaction type requires a JTA data source to be provided. The [hotspot=8]`<jta-data-source />` 
...
The [hotspot=10-11]`eclipselink.ddl-generation` properties are used here so that you aren't required to 
...
[hotspot=15]`@PersistenceContext` annotation. The entity manager interacts with the persistence context. 
...
The [hotspot=13-44]`EventDao` class has a method for each CRUD operation, so let's break them down:

* The [hotspot=18-20]`createEvent()` method persists an instance of the `Event` entity class to the data store by
calling the [hotspot=19]`persist()` method on an EntityManager instance. The entity instance becomes managed
and changes to it will be tracked by the entity manager.

* The [hotspot=22-24]`readEvent()` method returns an instance of the `Event` entity class with the specified primary 
key by calling the [hotspot=23]`find()` method on an `EntityManager` instance. If the event instance is found, it
is returned in a managed state, but, if the event instance is not found, `null` is returned.

* The [hotspot=34-36]`readAllEvents()` method demonstrates an alternative way to retrieve event objects 
...
Similarly, the [hotspot=38-43]`findEvent()` method uses the `Event.findEvent` named query to find an event 
with the given name, location and time. 

* The [hotspot=26-28]`updateEvent()` method creates a managed instance of a detached entity instance. 
...
you must merge that entity into the persistence context by calling the [hotspot=27]`merge()` method 
so that changes to loaded fields of the detached entity are tracked.

* The [hotspot=30-32]`deleteEvent()` method removes an instance of the `Event` entity class from the database by 
calling the [hotspot=31]`remove()` method on an `EntityManager` instance. The state of the entity is
changed to removed and is removed from the database upon transaction commit. 

The DAO is injected into the `finish/backendServices/src/main/java/io/openliberty/guides/event/resources/EventResource.java`
class and used to access and persist data. The `@Transactional` annotation is used in the
`EventResource` class to declaratively control the transaction boundaries on the [hotspot=12]`@RequestScoped` CDI bean.
