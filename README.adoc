...
that are displayed in the WAS server log. To learn more, see the http://www.eclipse.org/eclipselink/documentation/2.5/jpa/extensions/p_logging_level.htm[
...
the data source that is to be used by the EJB container. This means that a Java Database Connectivity 
(JDBC) connection does not have to be specified in the `persistence.xml` file and instead the data 
source can be preconfigured in the `server.xml`.
...
* The `readEvent()` and `readAllEvents()` methods both return instances of the `Event` entity class from
the datastore. The `readEvent()` method returns a single instance with the specified primary key by
calling the `find()` method on an `EntityManager` instance. The `find()` method accepts both the entity
class and the primary key as arguments.  The `readAllEvents()` method will return a list of event 
entities using the `Event.findAll` query specified in the `@NamedQuery` annotation on the `Event` class. 
