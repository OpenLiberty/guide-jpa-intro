...
:page-seo-description: Learn how to access and persist data in microservices using the Java Persistence API.
...
that uses MicroProfile Rest Client to access RESTful services and an event microservice 
for creating, retrieving, updating, and deleting events. In this guide you will be focused 
on the event microservice. The database logic is implemented in a Data Access Object (DAO) 
to isolate the database operations from the rest of the application. This DAO accesses 
and persists data model instances to the database and can be injected and consumed by other 
components in the application. An Embedded Derby database is used as a data store for all the events.
...
context that is managed by an entity manager. In a Java EE environment, one type of entity
manager is a container-managed entity manager. In this guide you will use dependency injection
to acquire a container-managed entity manager, placing the burden of opening and closing 
the entity manager on the application server. 
...
The `Event` class is a data model that represents events in the application and is annotated with JPA
annotations. Create the `Event` class in the `backendServices/src/main/java/io/openliberty/guides/models/Event.java` file:
[source, Java]
----
include::finish/backendServices/src/main/java/io/openliberty/guides/models/Event.java[tags=!copyright;]
----
...
persistence unit defines the set of entities managed by the entity manager. 
...
To perform CRUD operations by using JPA, create the `backendServices/src/main/java/io/openliberty/guides/dao/EventDao.java` 
file:

[source, Java]
----
include::finish/backendServices/src/main/java/io/openliberty/guides/dao/EventDao.java[tags=!copyright;]
----
...
At the end of the transactional method invocation, the persistance context is notified 
that the transaction is committing, and flushes any changes to Event entity instances it 
is managing to the database. Note that after the transaction commits, managed entities 
become unmanaged (detached).
...
This method returns a list of instances of the `Event` entity class by using the 
`Event.findAll` query specified in the `@NamedQuery` annotation on the `Event` class. 
...
Create the `EventEntityTest` test class in the `backendServices/src/test/java/it/io/openliberty/guides/event/EventEntityTest.java` 
file:
...
The `testInvalidRead()`, `testInvalidDelete()` and `testInvalidUpdate` methods test reading, 
deleting and updating an event that does not exist, respectively, by using a primary key 
that does not exist in the database.

The `testIndividualRead()` method persists a test event to the database and retrieves the 
