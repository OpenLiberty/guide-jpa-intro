...
= Persisting and accessing data using Java Persistence API (JPA)
...
JPA is a Java specification for representing RDBMS table data as Plain Old Java Objects (POJO). The 
JPA specification is implemented by several different persistence providers. In this guide you will 
use EclipseLink as the persistence provider because it is the implementation that is shipped with Liberty.

JPA simplifies object relational mapping (ORM) by using annotations and/or XML to map Java objects 
...
detached at the end of transaction boundaries. Lastly, removed entities are ones that have been removed 
from the database. 
...
for changes to the detached entity's laoded fields to be tracked.
