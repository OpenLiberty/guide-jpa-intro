...
:page-seo-description: Tutorial and example on how to access and persist data in microservices using the Java Persistence API (JPA).
...
class and used to access and persist data. The `@Transactional` annotation in the `EventResource` class 
is used to declaratively control the transaction boundaries on the `@RequestScoped` CDI bean.
