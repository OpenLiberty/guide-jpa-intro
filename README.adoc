...
an event microservice for creating, retrieving, updating, and deleting events. The UI uses 
a bean which consumes a REST API to interact with the backend services. The database logic
is implemented in a Data Access Object (DAO) to isolate the database operations from the 
rest of the application. This DAO accesses and persists data model instances to the database
and can be injected and consumed by other components in the application. The application 
uses an Embedded Derby database as a data store for all the events.
...
burden of managing the lifecycle of an EntityManager instance on the application server. 
...
annotations. Create the `Event` class in the `src/main/java/io/openliberty/guides/jpa/models/Event.java` file:
[source, Java]
----
include::finish/src/main/java/io/openliberty/guides/jpa/models/Event.java[tags=!copyright;]
----
...
To perform CRUD operations by using JPA, create the `src/main/java/io/openliberty/guides/jpa/dao/EventDao.java` 
file:

[source, Java]
----
include::finish/src/main/java/io/openliberty/guides/jpa/dao/EventDao.java[tags=!copyright;]
----
...
manages a set of `Event` entities and is aware of the different states that an entity can have.
The persistence context then synchronizes with the database when a transaction commits.

The `@Transactional` annotation allows the application to manage transaction boundaries 
on CDI-managed beans. This transaction management ensures that the required methods run 
within the boundaries of an active global transaction, which is why it is not necessary 
to explicitly begin, commit or rollback transactions. At the end of the transactional 
method invocation, the persistance context is notified that the transaction is committing, 
and flushes any changes to `Event` entity instances it is managing to the database. Note
that after the transaction commits, managed entities become unmanaged (detached).
...
calling the `persist()` method on an EntityManager instance. The event instance becomes 
managed and changes to it are now tracked by the entity manager. 
...
calling the `remove()` method on an `EntityManager` instance. 
