...
because there are no events stored in the database. Go ahead and click `Create Event`, located in the 
...
removing entities. The configuration information of the EntityManager, such as the transaction type and 
...
and if the database should be created if it does not exist.
...
manages a set of entities and is aware of the different states that an entity can have. The EntityManager
provides methods for interacting with the persistence context which is synchronized to the database
during a transaction commit. 

Every entity object has a lifecycle that consists of four states: new, managed, detached and removed. 
When a new Event object is created its state will be new, meaning that it is not mapped to a row in 
...
To use the EntityManager at runtime, it must be injected into our CDI bean through the
...
into the persistence context by calling the `merge()` method so changes to the detached entity's loaded 
fields are tracked.
...
be persisted to the database. After you have created an event, it will be available to view, update, 
