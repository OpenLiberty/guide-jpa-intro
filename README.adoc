...
= Persisting and accessing data using JPA
...
database operations. This allows you to focus on the business logic of your application instead of 
wasting time implementing repetitive CRUD logic.
...
context that is managed by an EntityManager, which itself is injected into an Enterprise Java Bean
(EJB) Session Bean. In this guide you will learn how to use a transaction-scoped container-managed
persistence context EntityManager. This places the burden of managing the EntityManager's lifecycle
on the application server with a stateless EJB session bean. The session bean has been configured
so that the EJB container manages the global transaction boundaries, so that you do not need to 
explicitly begin, commit, or rollback database transactions. 
...
to use Java Transaction API (JTA) to manage the EntityManager's transactions. Since we are using
a container-managed EntityManager, it is required that JTA transactions are used. 
...
been persisted and are managed by the EntityManager. This means that they are actively tracked by the 
persistence context. Detached entities are no longer managed by an EntityManager, but still exist in the 
database. When using a container-managed EntityManager, entities automatically become detached at the end 
of transaction boundaries. Lastly, removed entities are ones that have been removed from the database. 
...
`@PersistenceContext` annotation. Since the stateless session bean is configured to manage the global
transactions, the EJB container will ensure that the required methods are executed within the boundaries
of an active global transaction, which is why it is not necessary to explicitly begin, commit or rollback
transactions. At the end of a session bean method invocation, the persistence context will be notified
that the transaction is committing, and will flush any changes made to entity instances it is managing
to the database.
...
class and the primary key as arguments. If the state of the entity being searched for is managed, the event 
object will be returned as is and no retrieval from the database is required. But, if it is not already managed,
a new entity object will be constructed with data retrieved from the database and returned in a managed state.
If the event instance is not found, `Null` will be returned.
...
into the persistence context by calling the `merge()` method on an `EntityManager` instance in order for its 
