...
The `testInvalidRead()` and `testInvalidDelete()` methods test reading and deleting an
event that does not exist, respectively, by using a primary key that does not exist in the database.
