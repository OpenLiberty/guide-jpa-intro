...
At the end of the transactional method invocation, the persistence context is notified 
...
The `testInvalidRead()`, `testInvalidUpdate()` and `testInvalidDelete` methods use a primary
key that does not exist in the database to test reading, updating and deleting an event
that does not exist, respectively.
