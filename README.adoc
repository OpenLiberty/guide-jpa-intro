...
annotations. Create the `Event` class in the `backendServices/src/main/java/io/openliberty/guides/event/models/Event.java` file:
[source, Java]
----
...
To perform CRUD operations by using JPA, create the `backendServices/src/main/java/io/openliberty/guides/event/dao/EventDao.java` 
file:
...
key that is not in the database to test reading, updating and deleting an event that does not
exist, respectively.
