...
creating, retrieving, updating and deleting events. The application uses an Embedded Derby 
database as a datastore for all the event entities. If you want to learn more about Embedded Derby, 
see the https://db.apache.org/derby/papers/DerbyTut/embedded_intro.html[Embedded Derby Tutorial.]
...
Add the JPA annotations to the `Event` class in the `src/main/java/io/openliberty/guides/jpaguide/models/Event.java`
file:
