...
In the [hotspot file=0]`pom.xml` configuration file, the [hotspot=copyDependencies]`copyDependencies` tag copies the specified third-party dependencies to a specified location. Multiple [hotspot=dependencies]`dependency` parameters can be added to the `copyDependencies` configuration. The [hotspot=location]`location` parameter can be added to the copyDependencies to override the default location, which is the lib folder of the target server. 
