...
burden of managing the lifecycle of the EntityManager on the application server. The lifetime of the
persistence context can either be transaction-scoped or extended when using a container-managed EntityManager.
In this guide you will be focused on using a container-managed EntityManager with a transaction-scoped
persistence context to take advantage of the application server's services.
