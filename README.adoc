...
:page-seo-title: Accessing and persisting data in microservices
:page-seo-description: A tutorial and example on how to access and persist data in microservices using the Java Persistence API (JPA).
...
[role="code_command hotspot=17-70", subs="quotes"]
----
...
----
include::finish/backendServices/src/main/java/io/openliberty/guides/event/models/Event.java[tags=**;!copyright;]
----
...
| [hotspot=12]`@Entity` | Declares the class as an entity
| [hotspot=13]`@Table`  | Specifies details of the table such as name 
| [hotspot=14-16]`@NamedQuery` | Specifies a predefined database query that is run by an EntityManager instance.
| [hotspot=21]`@Id`       |  Declares the primary key of the entity
| [hotspot=20]`@GeneratedValue`    | Specifies the strategy used for generating the value of the primary key. 
                        The `strategy = GenerationType.AUTO` code indicates that the generation strategy 
                        is automatically selected
                        
| [hotspot=22]`@Column`    | Specifies that the field is mapped to a column in the database table. The `name`
...
[role="code_command hotspot=1-14", subs="quotes"]
----
...
----
include::finish/backendServices/src/main/resources/META-INF/persistence.xml[tags=**;]
----

The persistence unit is defined by the [hotspot=7-13 file=0]`<persistence-unit/>` XML element. The `name` attribute is 
required and is used to identify the persistent unit when using the `@PersistenceContext`
annotation to inject the entity manager later in this guide. The [hotspot=7 file=0]`transaction-type="JTA"` 
...
A JTA transaction type requires a JTA data source to be provided. The [hotspot=8 file=0]`<jta-data-source/>` 
...
in the [hotspot=1-28 file=1]`backendServices/src/main/liberty/config/server.xml` file. This data source configuration is where 
...
[source, Xml, linenums, role='code_column']
----
include::finish/backendServices/src/main/liberty/config/server.xml[tags=**]
----

The [hotspot=10-11 file=0]`eclipselink.ddl-generation` properties are used here so that you aren't required to 
...
The CRUD operations are defined in the DAO. To perform these operations by using JPA, 

[role="code_command hotspot=13-44", subs="quotes"]
----
...
[source, Java, linenums, role='code_column']
----
include::finish/backendServices/src/main/java/io/openliberty/guides/event/dao/EventDao.java[tags=!copyright;]
----

To use the entity manager at runtime, inject it into our CDI bean through the
[hotspot=15 file=0]`@PersistenceContext` annotation. The entity manager interacts with the persistence context. 
...
The [hotspot=13-44 file=0]`EventDao` class has a method for each CRUD operation, so let's break them down:

* The [hotspot=18-20 file=0]`createEvent()` method persists an instance of the `Event` entity class to the data store by
calling the [hotspot=19 file=0]`persist()` method on an EntityManager instance. The entity instance becomes managed
and changes to it will be tracked by the entity manager.

* The [hotspot=22-24 file=0]`readEvent()` method returns an instance of the `Event` entity class with the specified primary 
key by calling the [hotspot=23 file=0]`find()` method on an `EntityManager` instance. If the event instance is found, it
is returned in a managed state, but, if the event instance is not found, `null` is returned.

* The [hotspot=34-36 file=0]`readAllEvents()` method demonstrates an alternative way to retrieve event objects 
from the database. This method returns a list of instances of the `Event` entity class by 
using the `Event.findAll` query specified in the [hotspot=14-15 file=1]`@NamedQuery` annotation on the [hotspot=17-70 file=1]`Event` class. 
Similarly, the [hotspot=38-43 file=0]`findEvent()` method uses the `Event.findEvent` named query to find an event 
...
* The [hotspot=26-28 file=0]`updateEvent()` method creates a managed instance of a detached entity instance. 
...
you must merge that entity into the persistence context by calling the [hotspot=27 file=0]`merge()` method 
so that changes to loaded fields of the detached entity are tracked.

* The [hotspot=30-32 file=0]`deleteEvent()` method removes an instance of the `Event` entity class from the database by 
calling the [hotspot=31 file=0]`remove()` method on an `EntityManager` instance. The state of the entity is
changed to removed and is removed from the database upon transaction commit. 

The DAO is injected into the [hotspot=28-128 file=2]`finish/backendServices/src/main/java/io/`
[hotspot=28-128 file=2]`openliberty/guides/event/resources/EventResource.java`
class and used to access and persist data. The [hotspot=39 file=2]`@Transactional` annotation is used in the
[hotspot=28-128]`EventResource` class to declaratively control the transaction boundaries on the [hotspot=12 file=0]`@RequestScoped` CDI bean.
...
[source, Java, linenums, role='code_column']
----
include::finish/backendServices/src/main/java/io/openliberty/guides/event/resources/EventResource.java[tags=**;!copyright]
----
...
[role="code_command hotspot=18-124", subs="quotes"]
----
...
[source, Java, linenums, role='code_column']
----
include::finish/backendServices/src/test/java/it/io/openliberty/guides/event/EventEntityTest.java[tags=!copyright;]
----

The [hotspot=54-57]`testInvalidRead()`, [hotspot=60-64]`testInvalidDelete()` and [hotspot=67-71]`testInvalidUpdate()` methods use a primary key that is not in the database to test reading, updating and deleting an event that does not
exist, respectively.

The [hotspot=74-87]`testIndividualRead()` method persists a test event to the database and retrieves the 
event object from the database using the primary key of the entity.

The [hotspot=90-116]`testCRUD()` method creates a test event and persists it to the database. The event object is then 
...
instances are represented as rows in the table in Open Liberty. You have injected a container-managed 
entity manager into a DAO and learned how to perform CRUD operations in your microservice.
