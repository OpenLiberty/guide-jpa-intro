...
shell sessions where you ran the `backendServices` and `frontendUI` services.
Alternatively, you can run the `liberty:stop` goal from the finish directory in another shell session for the `frontendUI`
...
You will need two command-line instances to start the two services in development mode.
